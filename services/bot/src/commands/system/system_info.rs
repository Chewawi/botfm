...
pub async fn system_info(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let process_info = sys.process(Pid::from(pid_u32 as usize))
        .map(|p| format!(
            "### Process Info\n\
            - Name: `{:?}`\n\
            -#\t- PID: `{}`\n\
    ...
    // Disks
    let disk_info = disks.list().iter().map(|d| {
    ...
    let container = serenity::CreateContainer::new(vec![
        serenity::CreateComponent::TextDisplay(serenity::CreateTextDisplay::new(ascii_tree)),
    ]);
...
fn generate_ascii_tree(
    ...
    // Hostname and OS
    writeln!(&mut tree, "{}├── {}Hostname: {}{}{}{}", main, label, bold, value, hostname, reset).unwrap();
    writeln!(&mut tree, "{}├── {}Operating System{}", main, label, reset).unwrap();
    writeln!(&mut tree, "{}│   ├── {}Name: {}{}{}{}", main, label, bold, value, os_name, reset).unwrap();
    writeln!(&mut tree, "{}│   ├── {}Version: {}{}{}{}", main, label, bold, value, os_version, reset).unwrap();
    writeln!(&mut tree, "{}│   └── {}Kernel: {}{}{}{}", main, label, bold, value, kernel_version, reset).unwrap();

    // CPU
    writeln!(&mut tree, "{}├── {}CPU{}", main, label, reset).unwrap();
    writeln!(&mut tree, "{}│   ├── {}Count: {}{}{}{}", main, label, bold, value, cpu_count, reset).unwrap();

    // Color CPU usage based on value
    let cpu_color = if global_cpu > 80.0 { red } else if global_cpu > 50.0 { yellow } else { green };
    writeln!(&mut tree, "{}│   └── {}Usage: {}{}{:.2}%{}", main, label, bold, cpu_color, global_cpu, reset).unwrap();

    // Memory
    writeln!(&mut tree, "{}├── {}Memory{}", main, label, reset).unwrap();
    writeln!(&mut tree, "{}│   ├── {}RAM: {}{}{} / {}{}", main, label, bold, value, used_memory, total_memory, reset).unwrap();
    writeln!(&mut tree, "{}│   └── {}Swap: {}{}{} / {}{}", main, label, bold, value, used_swap, total_swap, reset).unwrap();

    // Network
    writeln!(&mut tree, "{}├── {}Network{}", main, label, reset).unwrap();
    writeln!(&mut tree, "{}│   ├── {}Received: {}{}{}{}", main, label, bold, value, humanize!(rx), reset).unwrap();
    writeln!(&mut tree, "{}│   └── {}Transmitted: {}{}{}{}", main, label, bold, value, humanize!(tx), reset).unwrap();
    ...
    if let Some(name_line) = process_info.lines().nth(1) {
        if let Some(name) = name_line.trim().strip_prefix("- Name: ") {
            writeln!(&mut tree, "{}│   ├── {}Name: {}{}{}{}", main, label, bold, value, name.trim_matches('`'), reset).unwrap();
        }
    }
    if let Some(pid_line) = process_info.lines().nth(2) {
        if let Some(pid) = pid_line.trim().strip_prefix("-#\t- PID: ") {
            writeln!(&mut tree, "{}│   ├── {}PID: {}{}{}{}", main, label, bold, value, pid.trim_matches('`'), reset).unwrap();
        }
    }
    if let Some(ram_line) = process_info.lines().nth(3) {
        if let Some(ram) = ram_line.trim().strip_prefix("- RAM: ") {
            writeln!(&mut tree, "{}│   ├── {}RAM: {}{}{}{}", main, label, bold, value, ram.trim_matches('`'), reset).unwrap();
        }
    }
    if let Some(cpu_line) = process_info.lines().nth(4) {
        if let Some(cpu) = cpu_line.trim().strip_prefix("- CPU: ") {
            ...
            let cpu_color = if cpu_num > 80.0 { red } else if cpu_num > 50.0 { yellow } else { green };
            writeln!(&mut tree, "{}│   ├── {}CPU: {}{}{}{}", main, label, bold, cpu_color, cpu_val, reset).unwrap();
        }
    }
    if let Some(status_line) = process_info.lines().nth(5) {
        if let Some(status) = status_line.trim().strip_prefix("- Status: ") {
            writeln!(&mut tree, "{}│   ├── {}Status: {}{}{}{}", main, label, bold, value, status.trim_matches('`'), reset).unwrap();
        }
    }
    if let Some(uptime_line) = process_info.lines().nth(6) {
        if let Some(uptime) = uptime_line.trim().strip_prefix("- Uptime: ") {
            writeln!(&mut tree, "{}│   └── {}Uptime: {}{}{}{}", main, label, bold, value, uptime.trim_matches('`'), reset).unwrap();
        }
    }
            ...
            if let Some(first_line) = disk_line.lines().next() {
                if let Some(name_start) = first_line.find("**") {
                    if let Some(name_end) = first_line[name_start+2..].find("**") {
                        ...
                        writeln!(&mut tree, "{}{}{}{}Disk: {}{}{}{}", main, prefix, bold, value, name, reset, main, reset).unwrap();
                        ...
                        if let Some(fs_start) = first_line.find("(`") {
                            if let Some(fs_end) = first_line[fs_start+2..].find("`)") {
                                ...
                                writeln!(&mut tree, "{}│   {}    ├── {}Filesystem: {}{}{}{}", main, main, label, bold, value, fs, reset).unwrap();
                            }
                        }
                        ...
                        if let Some(second_line) = disk_line.lines().nth(1) {
                            if let Some(usage_start) = second_line.find("`") {
                                if let Some(usage_end) = second_line[usage_start+1..].find("`") {
                                    let usage = &second_line[usage_start+1..usage_start+1+usage_end];
                                    let usage_num = usage.trim_end_matches('%').parse::<f32>().unwrap_or(0.0);
                                    let usage_color = if usage_num > 90.0 { red } else if usage_num > 70.0 { yellow } else { green };
                                    writeln!(&mut tree, "{}│   {}    └── {}Usage: {}{}{}{}", main, main, label, bold, usage_color, usage, reset).unwrap();
                                }
                            }
                        }
                    }
    ...
    if let Some(started_line) = bot_info.lines().nth(2) {
        if let Some(started) = started_line.trim().strip_prefix("-#\t- Started: ") {
            writeln!(&mut tree, "{}    ├── {}Started: {}{}{}{}", main, label, bold, value, started.trim_matches('`'), reset).unwrap();
        }
    }
